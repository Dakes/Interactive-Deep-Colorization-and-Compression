...
def init_reading(input, output, target_size):
    ...
    if 'leftImg8bit' in input:
        ...
        cityscapes.convert_to_gan_reading_format_save(input + 'train/', output + 'train/', target_size)
        cityscapes.convert_to_gan_reading_format_save(input + 'val/', output + 'val/', target_size)
        cityscapes.convert_to_gan_reading_format_save(input + 'test/', output + 'test/', target_size)
    else:
